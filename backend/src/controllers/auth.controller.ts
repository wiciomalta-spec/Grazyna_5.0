...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!validation.success) {
      res.status(400).json({ error: "Błąd walidacji", details: validation.error.flatten() });
      return;
    }
...
export const me = async (req: Request & { user?: { id: string } }, res: Response): Promise<void> => {
  ...
  try {
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, email: true, username: true, firstName: true, lastName: true, role: true, lastLoginAt: true, createdAt: true }
    });
    if (!user) {
