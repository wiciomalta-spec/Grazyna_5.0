...
const minWorkers = 1;
const maxWorkers = Math.max(1, Math.min(Number(process.env.MAX_CLUSTER_WORKERS || cpuCount), cpuCount));
let currentWorkers = Math.max(minWorkers, Math.min(Number(process.env.CLUSTER_WORKERS || Math.max(1, cpuCount - 1)), maxWorkers));
...
if (cluster.isPrimary) {
  ...
  const controlServer = http.createServer((req, res) => {
    ...
    if (req.url === "/scale-down") {
      ...
      res.end("SCALED DOWN -> " + Math.max(1, currentWorkers));
      return;
    }

    if (req.url === "/status") {
      res.setHeader("Content-Type", "application/json");
      res.end(JSON.stringify({ workers: workers.length, currentWorkers, minWorkers, maxWorkers }));
      return;
    }
