...
app.get('/metrics', async (_req, res) => {
    try {
        ...
        const ensureMetric = (name, exposition) => {
...
try {
    ...
    const rawMetrics = register.getMetricsAsJSON
        ? register.getMetricsAsJSON()
        : [];
    const metricNames = Array.isArray(rawMetrics)
        ? rawMetrics.map((m) => m?.name).filter(Boolean)
        : [];
...
try {
    ...
    function makeIfMissing(name, creator) {
        try {
            const existing = reg.getSingleMetric ? reg.getSingleMetric(name) : null;
            if (!existing) {
                const m = creator();
                // register if creator didn't already
                if (reg.registerMetric && m && !(reg.getSingleMetric && reg.getSingleMetric(name))) {
    ...
    try {
        reg.getSingleMetric('http_requests_total')?.inc?.({ method: '_init', route: '/_init', status: '0' }, 0);
    }
    catch (e) { }
    try {
        reg.getSingleMetric('http_request_duration_seconds')?.observe?.({ method: '_init', route: '/_init', status: '0' }, 0);
    }
    catch (e) { }
    try {
        reg.getSingleMetric('http_errors_total')?.inc?.({ method: '_init', route: '/_init', status: '0' }, 0);
    }
    catch (e) { }
    try {
        reg.getSingleMetric('worker_processes')?.set?.(Number(process.env.WORKER_COUNT || 1));
    }
    catch (e) { }
}
catch (e) {
