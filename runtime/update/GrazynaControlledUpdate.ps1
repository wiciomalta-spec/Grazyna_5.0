...
$Backend=Join-Path $Root 'backend'; $Frontend=Join-Path $Root 'frontend'
...
Write-Log INFO "GRAŻYNA CONTROLLED UPDATE — action=$Action targets=$($Targets -join ',') root=$Root"
...
if($Action -eq 'Apply'){
  if($Confirm -ne 'GRAZYNA-APPLY'){throw 'CONFIRMATION_REQUIRED: wymagane -Confirm GRAZYNA-APPLY'}
...
Write-Log INFO "APPLY zatwierdzone plan_id=$PlanId root=$Root";$results=@()
...
[ordered]@{status=if(@($results|Where-Object status -eq 'FAIL').Count -eq 0){'PASS'}else{'FAIL'};plan_id=$PlanId;root=$Root;results=$results;log=$log}|ConvertTo-Json -Depth 8
