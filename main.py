...
if __name__ == "__main__":
    log("=== START GRAŻYNA 5.0 ===")
    root = tk.Tk()
    app = GrazynaSystem(root)
    root.mainloop()

# === AUTOCOMPLETE RUNTIME PATCH START ===

def grazyna_autocomplete(user_input):

    suggestions = autocomplete_engine.suggest(user_input)

    print(f"[AUTO] Suggestions: {suggestions}")

    action = intent_parser.resolve(user_input)

    if action:
        print(f"[AUTO] Action detected: {action}")

    return suggestions, action

# === AUTOCOMPLETE RUNTIME PATCH END ===
